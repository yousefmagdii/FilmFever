...
function TVShow() {
  ...
  //   return (
  //     <>
  //       <div className="h-dvh text-center  font-truculenta text-2xl text-white">
  //         <span className="my-auto">No TV Shows Found</span>
  //       </div>
  //     </>
  //   );
  return (
    <>
      ...
      <div
        ...
        ${isFilterClicked ? 'pointer-events-none mt-9 opacity-40' : ''} 
          ${shows.length < 6 ? '!h-dvh overflow-hidden' : '!h-full'}`}
      >
        {shows.length > 0 ? (
                ...
                >
                  <LazyLoad height={400} offset={100}>
                    <img
                      ...
                      alt={show.name}
                      className={`h-90 w-64 rounded-lg bg-cover object-cover ${isFilterClicked ? 'cursor-no-drop' : ''}`}
                    />
                  </LazyLoad>
                </Link>
                <span className="absolute bottom-0 left-0 right-0 z-10 hidden bg-gradient-to-t from-[#121212ac] to-[#00000000] py-5 text-center font-bold text-orange-50 group-hover:block group-hover:rounded-b-lg group-hover:duration-700 max-xl:block">
          ...
          <div className="flex h-screen items-center justify-center text-center text-4xl text-gray-200">
            There are no TV shows available please change the genres.
          </div>
        )}
      </div>
